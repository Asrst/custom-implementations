...
import logging
import numpy as np
from model_utils import ModelType, NLP_MODELS, TaskType, LOSSES
from transformers import AdamW, get_linear_schedule_with_warmup
logger = logging.getLogger("multi-head-nlp")
...
class multiHeadNetwork(nn.Module):
    def __init__(self, params):
        ...
        assert self.taskParams.modelType in ModelType._value2member_map_, "Model Type is NOT recognized!"
        self.modelType = self.taskParams.modelType


        # Initialize a shared base encoder model
        # Initializing with a config file doesn't load the model weights,
        # only the configuration. Use from_pretrained() to load the model weights.
    ...
    def make_multitask_heads(self):
        ...
        for taskId, taskName in self.taskParams.taskIdNameMap.items():
            ...
            dropoutLayer = nn.Dropout(dropoutValue)
...
class multiHeadModel:
    '''
    This class handles model architecture, training and saving. It has following functions

    1. Make the multi-head network for different tasks
    ...
    def __init__(self, params):
        ...
        if torch.cuda.device_count() > 1:
            ...
            self.network = nn.DataParallel(multiHeadNetwork(params))
        else:
            self.network = multiHeadNetwork(params)
    ...
    def make_optimizer(self, numTrainSteps, lr, eps, warmupSteps=0):
        # we will use AdamW optimizer from huggingface transformers. 
    ...
    def save_multi_task_model(self, savePath):
        '''
        Save the model parameters with state dict with current optimizer state, 
        along with the current global_steps and epoch which would help for resuming training
        Plus save the task parameters object created from the task file.
    ...
    def load_shared_model(self, loadedDict, freeze):
        ...
        logger.info('Searching for common parameters for task specific headers...')
